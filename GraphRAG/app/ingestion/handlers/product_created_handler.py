...
from app.ingestion.transformers import transform_product_created
...
class ProductCreatedHandler:
    ...
    async def handle(self, payload: dict) -> None:
        ...
        logger.info("Handling ProductCreated", product_id=product_id)

        document = transform_product_created(payload)

        await self._vector_store.upsert_product(document)
        await self._graph_repo.upsert_product(document)

        logger.info("ProductCreated processed", product_id=product_id)
